...
def classificador(alpha, treino_tam, teste_tam, folds_tam, fit_prior, divisao, vec_tipo, etiqueta, caminho, caminho_motores):
    ...
    multinomialnaiveb = MultinomialNB(alpha=alpha, fit_prior=fit_prior)
    ...
    if (divisao == 'percent_split'):
        ...
        if (vec_tipo == 'countvec'):
            vec_clf = Pipeline([('vectorizer', count_vect), ('clf', multinomialnaiveb)])

        if (vec_tipo == 'tdidf_transf'):
            tfidf_transformer = TfidfVectorizer()
            vec_clf = Pipeline([('vectorizer', tfidf_transformer), ('clf', multinomialnaiveb)])
        ...
        pickle.dump(vec_clf, open(caminho_motores + 'multinomialNB_@' + etiqueta.replace('.csv','') + '.sav', 'wb'))
...
# Alpha:
    # Um numero qualquer maior que zero.
...
# classificador(alpha=1, fit_prior=True, divisao='cross_val', treino_tam=0.66, teste_tam=0.33, folds_tam=5, vec_tipo='countvec')