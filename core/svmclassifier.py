...
def classificador(gamma, c, treino_tam, teste_tam, folds_tam, kernel, divisao, vec_tipo, etiqueta, caminho, caminho_motores):
    reload(sys)
    ...
    if (divisao == 'percent_split'):
        ...
        if (vec_tipo == 'countvec'):
            vec_clf = Pipeline([('vectorizer', count_vect), ('clf', svm)])

        if (vec_tipo == 'tdidf_transf'):
            tfidf_transformer = TfidfVectorizer()
            vec_clf = Pipeline([('vectorizer', tfidf_transformer), ('clf', svm)])
        ...
        pickle.dump(vec_clf, open(caminho_motores + 'svm_@' + etiqueta.replace('.csv','') + '.sav', 'wb'))
    ...
    # A ordem da lista "resultados_treinamento" será sempre: [accuracy_value, precision_value, recall_value, f1_value, roc_value]
    return resultados_treinamento, tamanho_da_lista, caminho_motores

#OPÇÕES

# Gamma:
    # Qualquer número que o usuário desejar, além da palavra "auto", que sinaliza para o algoritmo que ele deve
    #fazer o cálculo 1/(n_features * X.std()) para chegar ao valor de Gamma.
...
# classificador(gamma='auto', kernel='rbf', c=1.0, divisao='percent_split', treino_tam=0.66, teste_tam=0.33, folds_tam=5, vec_tipo='countvec')