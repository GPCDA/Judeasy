...
def classificador(n_estimators, max_depth, treino_tam, teste_tam, folds_tam, criterion, divisao, vec_tipo, etiqueta, caminho, caminho_motores):
    ...
    randomforest = RandomForestClassifier(n_estimators=n_estimators, criterion=criterion, max_depth=max_depth)
    ...
    if (divisao == 'percent_split'):
        ...
        if (vec_tipo == 'countvec'):
            vec_clf = Pipeline([('vectorizer', count_vect), ('clf', randomforest)])

        if (vec_tipo == 'tdidf_transf'):
            tfidf_transformer = TfidfVectorizer()
            vec_clf = Pipeline([('vectorizer', tfidf_transformer), ('clf', randomforest)])
        ...
        pickle.dump(vec_clf, open(caminho_motores + 'randomForest_@' + etiqueta.replace('.csv','') + '.sav', 'wb'))
...
# N_estimators:
    # Um numero INTEIRO qualquer, maior que zero.
...
# classificador(n_estimators=10, criterion='gini', max_depth=None, divisao='percent_split', treino_tam=0.66, teste_tam=0.33, folds_tam=5, vec_tipo='tdidf_transf')