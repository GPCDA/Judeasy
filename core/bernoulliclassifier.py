...
def classificador(alpha, treino_tam, teste_tam, folds_tam, divisao, vec_tipo, etiqueta, caminho, caminho_motores):
    ...
    if (divisao == 'percent_split'):
        ...
        if (vec_tipo == 'countvec'):
            vec_clf = Pipeline([('vectorizer', count_vect), ('clf', bernoulli)])

        if (vec_tipo == 'tdidf_transf'):
            tfidf_transformer = TfidfVectorizer()
            vec_clf = Pipeline([('vectorizer', tfidf_transformer), ('clf', bernoulli)])
        ...
        pickle.dump(vec_clf, open(caminho_motores + 'bernoulli_@' + etiqueta.replace('.csv', '') + '.sav', 'wb'))
        ...
        #Salva em disco o modelo treinado para fazer predições posteriormente na tela de "Teste/Validação"
        #pickle.dump(bernoulli, open('modelobernoulli.sav', 'wb'))
...
# Alpha:
    # Qualquer número que o usuário desejar, inclusive 0 (zero), apesar de não ser o ideal.
    #PADRÃO: 1
...
#classificador(alpha=1, divisao='cross_val', treino_tam=0.66, teste_tam=0.33, folds_tam=5, vec_tipo='tdidf_transf')