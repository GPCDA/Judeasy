...
def classificador(max_iter, treino_tam, teste_tam, folds_tam, loss, penalty, divisao, vec_tipo, etiqueta, caminho, caminho_motores):
    ...
    sgdclass = SGDClassifier(loss=loss, penalty=penalty, max_iter=max_iter)
    ...
    if (divisao == 'percent_split'):
        ...
        if (vec_tipo == 'countvec'):
            vec_clf = Pipeline([('vectorizer', count_vect), ('clf', sgdclass)])

        if (vec_tipo == 'tdidf_transf'):
            tfidf_transformer = TfidfVectorizer()
            vec_clf = Pipeline([('vectorizer', tfidf_transformer), ('clf', sgdclass)])
        ...
        pickle.dump(vec_clf, open(caminho_motores + 'sgd_@' + etiqueta.replace('.csv','') + '.sav', 'wb'))
...
# classificador(loss='hinge', penalty ='l2', max_iter=1000, divisao='cross_val', treino_tam=0.66, teste_tam=0.33, folds_tam=5, vec_tipo='tdidf_transf')