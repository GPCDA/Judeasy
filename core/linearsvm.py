...
def classificador(c, treino_tam, teste_tam, folds_tam, penalty, loss, divisao, vec_tipo, etiqueta, caminho, caminho_motores):
    ...
    linearsvm = LinearSVC(penalty=penalty, loss=loss, C=c)
    ...
    if (divisao == 'percent_split'):
        ...
        if (vec_tipo == 'countvec'):
            vec_clf = Pipeline([('vectorizer', count_vect), ('clf', linearsvm)])

        if (vec_tipo == 'tdidf_transf'):
            tfidf_transformer = TfidfVectorizer()
            vec_clf = Pipeline([('vectorizer', tfidf_transformer), ('clf', linearsvm)])
        ...
        pickle.dump(vec_clf, open(caminho_motores + 'linearSVM_' + etiqueta.replace('.csv','') + '.sav', 'wb'))
...
# classificador(penalty='l2', loss='squared_hinge', c=1.0, divisao='cross_val', treino_tam=0.66, teste_tam=0.33, folds_tam=5, vec_tipo='tdidf_transf')