...
def classificador(k, treino_tam, teste_tam, folds_tam, metrica, divisao, vec_tipo, etiqueta, caminho, caminho_motores):
    ...
    if (divisao == 'percent_split'):
        ...
        if (vec_tipo == 'countvec'):
            vec_clf = Pipeline([('vectorizer', count_vect), ('clf', knn)])

        if (vec_tipo == 'tdidf_transf'):
            tfidf_transformer = TfidfVectorizer()
            vec_clf = Pipeline([('vectorizer', tfidf_transformer), ('clf', knn)])
        ...
        pickle.dump(vec_clf, open(caminho_motores + 'knn_@' + etiqueta.replace('.csv','') + '.sav', 'wb'))
        ...
        #Salva em disco o modelo treinado para fazer predições posteriormente na tela de "Teste/Validação"
        #pickle.dump(knn, open('modeloknn.sav', 'wb'))
...
#OPÇÕES
# K:
    # Qualquer número INTEIRO, a partir de 1 que o usuário desejar
    #PADRÃO: 5
...
# classificador(k=5, metrica='euclidean', divisao='cross_val', treino_tam=0.33, teste_tam=0.66, folds_tam=5, vec_tipo='tdidf_transf')