...
def classificador(hidden_layer_sizes, treino_tam, teste_tam, folds_tam, learning_rate_init, activation, solver, divisao, learning_rate, vec_tipo, etiqueta, caminho, caminho_motores):
    ...
    multilayerperceptron = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, activation=activation, solver=solver,
    ...
    if (divisao == 'percent_split'):
        ...
        if (vec_tipo == 'countvec'):
            vec_clf = Pipeline([('vectorizer', count_vect), ('clf', multilayerperceptron)])

        if (vec_tipo == 'tdidf_transf'):
            tfidf_transformer = TfidfVectorizer()
            vec_clf = Pipeline([('vectorizer', tfidf_transformer), ('clf', multilayerperceptron)])
        ...
        pickle.dump(vec_clf, open(caminho_motores + 'multilayerPerceptron_@' + etiqueta.replace('.csv','') + '.sav', 'wb'))
...
# classificador(hidden_layer_sizes=100, activation='relu', solver ='adam', learning_rate = 'constant', learning_rate_init=0.001, divisao='percent_split', treino_tam=0.66, teste_tam=0.33, folds_tam=5, vec_tipo='tdidf_transf')